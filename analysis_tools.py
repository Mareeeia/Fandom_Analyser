...
class FandomAnalysisTools:
    ...
    def plot_bars_time(self, data):
        print('Plotting data vs months')
    ...
    def plot_new_works_count_by_month_ratings(self, d):
        ...
        p5 = ax.bar(d5.index, d5['count_y'], width=25, align='center', color='tab:green')
        p4 = ax.bar(d4.index, d4['count_y'], width=25, bottom=d5['count_y'], align='center', color='tab:blue')
        p3 = ax.bar(d3.index, d3['count_y'], width=25, bottom=d5['count_y'] + d4['count_y'], align='center',
                    color='tab:orange')
        p1 = ax.bar(d1.index, d1['count_y'], width=25, bottom=d5['count_y'] + d4['count_y'] + d3['count_y'],
                    align='center', color='tab:red')
        p2 = ax.bar(d2.index, d2['count_y'], width=25,
                    bottom=d5['count_y'] + d4['count_y'] + d3['count_y'] + d1['count_y'], align='center',
                    color='tab:purple')
    ...
    def plot_average_hits_ships_character(self, d, char):
        ...
        plt.subplots_adjust(bottom=0.3)
        plt.draw()
        plt.savefig('fandom_extracted_data/' + self.fandom + '/plots/plot_average_hits_ships_character_' + char + '.png')
        plt.show()
    ...
    def plot_works_count_by_rating(self, d):
        ...
        df[is_large].plot(labels=df['Rating'], y='Fanfic Counts', kind='pie', autopct=self.make_autopct(df['Fanfic Counts']),
    ...
    def plot_works_count_by_category(self, d):
        ...
        plt.axis('off')
        plt.draw()
        plt.savefig('fandom_extracted_data/' + self.fandom + '/plots/plot_works_count_by_category.png')
        plt.show()
    ...
    def plot_new_char_works_count_by_month(self, d, char, show=True):
        ...
        plt.axhline(y=df['count'].mean(), color='r', linestyle='-')
        plt.draw()
        plt.savefig('fandom_extracted_data/' + self.fandom + '/plots/chars_bars_count/plot_new_works_count_by_month_' + char + '.png')
        if (show):
            print(df)
            plt.show()
        plt.close(fig)
    ...
    def main_character_ratio(self, d):
        ...
        print(df_combined.loc['Lamb'])
        is_large = df_combined['characters'] > 3
    ...
    def plot_char_common_appearances(self, d, char, show=True):
        ...
        plt.draw()
        plt.savefig('fandom_extracted_data/' + self.fandom + '/plots/chars_appearances/plot_character_appear_' + char + '.png')
        if (show):
            plt.show()
            print(char_df)
    ...
    def plot_char_shipping(self, d, char, show=True):
        ...
        if char_df.shape[0] > 0:
            ...
            nx.draw_networkx_labels(G, pos, labels, font_size=10)
            plt.draw()
            plt.savefig('fandom_extracted_data/' + self.fandom + '/plots/chars_ships/plot_character_ships_' + char + '.png')
            if (show):
                plt.show()
            for entry in char_df:
                G.add_node(entry)
    ...
    def plot_chars_everything(self, d):
        ...
        for char in charlist:
            ...
            plot_new_char_works_count_by_month(d, char, False)
            plot_char_shipping(d, char, False)
            plot_char_common_appearances(d, char, False)
            plot_char_tags(d, char, False)
    ...
    def champs_without_explicit(self, d):
        ...
        is_large = df['Fanfic Counts Without Explicit'] > 3
        plt.rc('font', size=8)
        plt.rc('axes', titlesize=8)
        df[is_large].plot(x='Characters', kind='bar', figsize=(20, 8))
